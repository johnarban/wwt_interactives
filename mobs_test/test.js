...
async function wwt_load_after_ready() {
    ...
    loadFitsFile('https://nova.astrometry.net/new_fits_file/8498818', 'Fits Image 1', opacity = 1)
...
function loadWTML2(filename, name, opacity = 1) {
    // check if aspx in filename
    loadChildFolders = filename.includes('.aspx');
    loadWTML(filename, name, () => { }, loadChildFolders)
    .then((imageset) => {
        // wwt_cl.setBackgroundImageByName(imageset._name);
        log(`${imageset._name} is loaded`, 'success')
...
function loadFitsFile(filename, name, opacity = 1) {
    asyncLoadFits(filename, name).then((imageset) => {
        // wwt_cl.setBackgroundImageByName(imageset._name);
        log(`${filename} is loaded`, 'success')
        imageset.set_name(name)
        imageset_layer = wwt_si.addImageSetLayer(imageset.url, "fits");
        // console.log(imageset_layer)
        imageset_layer.set_name(name);
        imageset_layer.opacity = opacity;
        imageset_layer.set_enabled(true);
        add_opacity_slider(imageset_layer);
        // wwt_si.setImageSetLayerOrder(imageset_layer.id, 1)
...
async function asyncLoadFits(filename, name) {
    wwt_si.loadFits(filename, name);
    return ensureImagesetLayerReady(name)
}
...
function ensureImagesetLayerReady(name) {
    // Create a promise that will be resolved when the imageset is ready
    return new Promise(function (resolve, reject) {
        (function waitForImagesetLayerReady() {
            log('ensuring ' + name + ' ImageSetLayer is ready', 'info')
            if (imagesetLayerExists(name)) return resolve(getImageSetLayerByName(name));
            setTimeout(waitForImagesetLayerReady, 100);
        })();
    });
}
...
function imagesetLayerExists(name) {
    // check if imageset is is registered with WWT
    perhaps_a_set = getImageSetLayerByName(name)
    exists = !(perhaps_a_set == null);
    if (exists) {
        // chdeck names match
        real_name = perhaps_a_set._name
...
function add_opacity_slider(layer) {
    ...
    span.onclick = () => {
        let layer2 = getImageSetLayerByName(layer._name)
        image = layer2.get_imageSet()
        ...
        if (layer2.opacity < .5) {
            input.value = 50;
            layer2.set_opacity(.5);
        } else {
            input.value = 100;
            layer2.set_opacity(1);
        }
    };
